...
get_file_list(char *pwd)
{
	...
	if (d) {
		...
		nftw(pwd, get_curr_file, FTW_D, FTW_PHYS);
	}
	closedir(d);
	return (0);
}
...
int 
get_curr_file(const char *base_name, const struct stat *info, const int typeflag, struct FTW *pathinfo)
{
...
int
add_file_to_memory(char *dirc, char *base_name, uintptr_t tfilesize)
{
	int		exists = 0;
...
int
check_existing(char *dirc, char *base_name, uintptr_t tfilesize)
{
	int		i = 0;
	int		existing = 0;
	int		low = 0;
